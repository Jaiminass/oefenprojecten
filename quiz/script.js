...
function showResult() {
  ...
  scoreText.innerText = `Je had ${score} goed en ${wrong} fout (${percentage.toFixed(1)}%)`;

  if (percentage >= 50) {
    resultTitle.innerText = "🎉 Geslaagd!";
    resultScreen.classList.add("pass");
    resultScreen.classList.remove("fail");
    document.getElementById("level2-btn").classList.remove("hidden"); // 🔥 toon Level 2 knop
  } else {
    resultTitle.innerText = "💥 Gezakt!";
    resultScreen.classList.add("fail");
    resultScreen.classList.remove("pass");
    document.getElementById("level2-btn").classList.add("hidden");
  }
